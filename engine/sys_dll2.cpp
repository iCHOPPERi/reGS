...
{
	// Allow overrides
	int nHeapSize = COM_CheckParm("-heapsize");
	if (nHeapSize && nHeapSize < com_argc - 1)
	{
		host_parms.memsize = atoi(com_argv[nHeapSize + 1]) * 1024;
	}
	...
	if (host_parms.memsize < MINIMUM_WIN_MEMORY)
	{
		...
		// but at least 14 Mb and no more than 128 Mb, unless they explicitly
		// request otherwise
		host_parms.memsize = lpBuffer.dwTotalPhys;

		if (host_parms.memsize)
		{
		...
		{
			// Just set default memory size
			host_parms.memsize = MAXIMUM_WIN_MEMORY;
		}

		if (g_bIsDedicatedServer)
		{
			// hard code 40 mb for dedicated servers
